      ...
      {section === "overview" && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <CoolMetric icon="◈" label="Expectancy" value={fmtMoney(k.expectancy, { sign: true })} sub={`${k.wins}W · ${k.losses}L · ${k.be}BE`} tone={k.expectancy >= 0 ? "gain" : "loss"} spark={[...daily.values()].map((d) => d.pnl)} />
            <CoolMetric icon="🔥" label="Streak" value={`${k.streak.len} ${k.streak.type}`} tone={k.streak.type === "win" ? "gain" : k.streak.type === "loss" ? "loss" : "brand"} sub={k.streak.type === "none" ? "no streak" : k.streak.type === "win" ? "keep discipline" : "size down"} spark={[...daily.values()].map((d) => (d.pnl > 0 ? 1 : d.pnl < 0 ? -1 : 0))} />
            <CoolMetric icon="◆" label="Best / Worst" value={`${fmtMoney(k.bestTrade)} / ${fmtMoney(k.worstTrade)}`} tone="brand" sub={`avg win ${fmtMoney(k.avgWin)} · avg loss ${fmtMoney(k.avgLoss)}`} spark={current.slice(-30).map((t) => t.pnl)} />
            <CoolMetric icon="⬢" label="Max Drawdown" value={fmtMoney(maxDd)} tone={maxDd > 0 ? "loss" : "brand"} sub={`${ddCurve.length} days tracked`} spark={ddCurve.slice(-30).map((d) => -d.dd)} />
          </div>
...
function CoolMetric({ icon, label, value, sub, tone, spark }: { icon: string; label: string; value: string; sub: string; tone: string; spark: number[] }) {
  if (!spark.length) {
    return (
